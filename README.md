# 👋 Hi, I'm Carmen Buendía

**Senior Full Stack Engineer** · React · TypeScript · Node.js · MongoDB · Docker · Kubernetes

📍 Remote (UTC+1) · 📞 +34 638955062 · ✉️ carmenbuendiafullstack@gmail.com

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/carmen-buendia)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white)](https://github.com/carmen-buendia)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:carmenbuendiafullstack@gmail.com)

---

## 💻 About Me

Software Engineer with **5+ years of experience** as a Full Stack Engineer, specializing in React, TypeScript, Node.js, and scalable architectures.

I have led legacy system migrations to modern SPAs, implemented micro-frontends in regulated banking environments, and built component libraries that reduced UI development time by up to 30%.

**🎵 Differentiator:** Background in music production and film composition. This gives me a unique perspective on communication, narrative, and sensitivity for digital products.

---

## 🛠️ Tech Stack

| Area | Technologies |
|------|--------------|
| **Frontend** | React, TypeScript, Next.js, Vue.js, Angular, Redux Toolkit, TailwindCSS, Styled Components, Chakra UI, Storybook |
| **Backend** | Node.js, Express.js, NestJS, REST APIs, Swagger |
| **Databases** | MongoDB, PostgreSQL, MariaDB, Prisma, TypeORM |
| **DevOps & CI/CD** | Docker, Kubernetes, Jenkins, GitHub Actions, ArgoCD |
| **Testing** | Jest, React Testing Library, Cypress |
| **Architecture** | Micro-frontends, Monorepos, Clean Architecture |

---

## 💼 Featured Experience

| Role | Company | Stack |
|------|---------|-------|
| Frontend Architect | CGI (Unicaja) | React, TypeScript, Micro-frontends, Docker, Kubernetes, Jenkins |
| Senior Frontend Developer | Laudus S.L. (Visma Latam) | React, TypeScript, AG-Grid, HighCharts, Storybook |
| Fullstack Freelance | International Clients | React, Next.js, NestJS, PostgreSQL, Stripe |

---

## 🌱 Featured Project

### Sicilia Soil · Permaculture & Mycology Dashboard

Real-time monitoring dashboard for a permaculture garden in Sicily. Integrates mushroom cultivation, esparto grass harvesting calendar, and IoT sensors.

- **Architecture:** Monorepo with full CI/CD
- **Stack:** React, Next.js, TypeScript, Tailwind, Socket.io, Node.js, GitHub Actions
- **Demonstrates:** Scalable architecture, real-time data, full-stack, automation

🔗 [View Repository](https://github.com/carmen-buendia/sicilia-soil)

---

## 📚 Education

- **Full Stack Web Developer Bootcamp (540h)** – The Bridge Digital Talent Accelerator (2020)
- **XXI Sound and Music for Film Workshop** – SGAE (2023-2024)
- **Software Design Patterns Course (150h)** – Levelcom (2021)

---

## 🌐 Languages

- **Spanish:** Native
- **English:** Advanced (fluent technical conversation)

---

## 🎧 More About Me

- Music producer and DJ with 20+ years of experience
- Collective founder: Carycar Club (Vigo), Melancholia_Collective (Madrid)
- Interests: permaculture, mycology, esparto grass, life in country.

---

## 📫 Contact

- **Email:** carmenbuendiafullstack@gmail.com
- **LinkedIn:** [linkedin.com/in/carmen-buendia](https://linkedin.com/in/carmen-buendia)
- **GitHub:** [github.com/carmen-buendia](https://github.com/carmen-buendia)

---

⭐ *"I don't just write code. I build solutions that matter."*
